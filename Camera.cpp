...
void camera_move(struct vector3<float> delta) {
	...
	if (camera[2] + delta[2] >= 0.01f) {
		...
		if (camera[0] + resolution[0] * (camera[2] + delta[2]) >= map_dimensions[0]) {
			if (resolution[0] * (camera[2] + delta[2]) < map_dimensions[0] && resolution[1] * (camera[2] + delta[2]) < map_dimensions[1]) {
				camera[0] = map_dimensions[0] - (unsigned int)ceilf(resolution[0] * (camera[2] + delta[2])) - 1;
			} else {
				delta_z_happens = false;
			}
		}
		if (camera[1] + resolution[1] * (camera[2] + delta[2]) >= map_dimensions[1]) {
			if (resolution[0] * (camera[2] + delta[2]) < map_dimensions[0] && resolution[1] * (camera[2] + delta[2]) < map_dimensions[1]) {
				camera[1] = map_dimensions[1] - (unsigned int)ceilf(resolution[1] * (camera[2] + delta[2])) - 1;
			} else {
				delta_z_happens = false;
			}
		}
	...
	if (camera[0] + delta[0] + resolution[0] * camera[2] >= map_dimensions[0]) {
		camera[0] = map_dimensions[0] - (unsigned int)ceilf(resolution[0] * camera[2]) - 1;
	} else {
		camera[0] += delta[0];
	}
	...
	if (camera[1] + delta[1] + resolution[1] * camera[2] >= map_dimensions[1]) {
		camera[1] = map_dimensions[1] - (unsigned int)ceil(resolution[1] * camera[2]) - 1;
	} else {
		camera[1] += delta[1];
	}
