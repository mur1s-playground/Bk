...
ThreadReturn inputThread(void* client)
{
    ...
    while(true)
    {
        ...
        if (command == "player_reg true") {
            player_reg_active = true;
            continue;
        }
        if (command == "player_reg false") {
            player_reg_active = false;
            continue;
        }
...
int main(int argc, char* argv[])
{
    ...
    if (argc < 6)
    {
        std::cout << "Insuficient parameters: host port [nick] [cache_dir] [twitch_name]" << std::endl;
        return 1;
    }
    ...
    if (argc >= 4)
        nick = argv[3];
    if (argc >= 5)
        cache_dir = std::string(argv[4]);
    if (argc >= 6) {
        twitch_name = std::string(argv[5]);
    }
